...
export default function AdminLeaderboardClient({ syncSchedule = [] }: { syncSchedule?: SyncTime[] }) {
  ...
  const [limit, setLimit] = useState(Number(searchParams.get('limit')) || 13);
  ...
  // Fixed podium — top 3 per city+year, never re-fetched on page/search changes
  const [podiumData, setPodiumData] = useState<LeaderboardData['leaderboard'] | null>(null);
  ...
  // Reset podium + page when city or year changes
  useEffect(() => {
    setPodiumData(null);
    setPage(1);
  }, [lCity, lYear]);
  ...
  const handleRefresh = async (force = false) => {
    ...
    try {
      ...
      if (debouncedPage === 1 && !search) {
        setPodiumData(response.leaderboard.slice(0, 3));
      }
    } catch (err: unknown) {
  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      ...
      try {
        ...
        // Lock podium to top 3 from the first unfiltered page-1 result per city+year
        if (debouncedPage === 1 && !search) {
          setPodiumData(prev => prev ?? response.leaderboard.slice(0, 3));
        }
      } catch (err: unknown) {
        // Error is handled by API client interceptor
  ...
  return (
    <div className="flex flex-col mx-auto  w-full pb-12  -m-3">
      ...
      {/* Mobile Podium - fixed to top 3, never re-shimmers on page/search changes */}
      ...
      {/* Desktop Podium - fixed to top 3, never re-shimmers on page/search changes */}
      ...
      <LeaderboardTable
        data={leaderboardData ? {
          ...leaderboardData,
          leaderboard: (page === 1 && !debouncedSearch) ? leaderboardData.leaderboard.slice(3) : leaderboardData.leaderboard,
        } : null}
